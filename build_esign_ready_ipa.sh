...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/VOLCANO-esign.XXXXXX")"
...
if [[ ! -x "$APP/VOLCANO" ]]; then
  echo "Error: expected original executable VOLCANO was not found in the app bundle." >&2
  exit 1
fi
