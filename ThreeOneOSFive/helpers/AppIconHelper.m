    ...
    dispatch_once(&onceToken, ^{
        ...
        for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (dlopen(candidates[i], RTLD_LAZY | RTLD_GLOBAL)) {
                NSLog(@"[VOLCANO] ls: loaded %s", candidates[i]);
                return;
            }
        }
        NSLog(@"[VOLCANO] ls: CoreServices/MobileCoreServices dlopen failed");
    });
}
...
static NSDictionary *appsFromWorkspace(void) {
    ...
    for (NSString *selectorName in @[@"allApplications", @"allInstalledApplications"]) {
        ...
        NSLog(@"[VOLCANO] ls: %@ returned %lu", selectorName,
              (unsigned long)([candidate isKindOfClass:[NSArray class]] ? [candidate count] : 0));
    }
    if (!apps || apps.count == 0) {
        NSLog(@"[VOLCANO] ls: workspace enumeration empty");
        return result;
    }
    NSLog(@"[VOLCANO] ls: %@ returned %lu proxies", usedSelector, (unsigned long)apps.count);
    ...
    NSLog(@"[VOLCANO] ls: extracted %lu apps (%lu with container)",
...
NSDictionary<NSString *, NSDictionary *> *installedAppInfo(void) {
    ...
    NSLog(@"[VOLCANO] ls: workspace empty; MobileInstallation=%lu",
