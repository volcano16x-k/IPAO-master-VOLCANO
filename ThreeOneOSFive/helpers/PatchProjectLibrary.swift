...
struct PatchLibraryItem: Identifiable {
    ...
    var displayName: String {
        ...
        if filename.hasPrefix("VOLCANO File (") {
            return filename
        }
...
enum PatchProjectLibrary {
    ...
    static func installBundledPackagesIfNeeded(
        ...
        guard let root = try? packageRootURL(
            fileManager: fileManager
        ) else {
            return
        }

        // Xcode may flatten folder references into the app bundle.
        // Resolve both the intended Patches subdirectory and the
        // flattened bundle root so standalone builds remain
        // self-contained across packaging layouts.
        ...
        for sourceURL in bundledURLs {
            ...
            do {
                ...
                log(
                    "patch: skipped bundled package \(sourceURL.lastPathComponent): \(error)"
                )
            }
        }
    }
    ...
    static func load(
        ...
        // Patch files use the .3105 extension.
        for url in urls
        where url.pathExtension.lowercased() == "3105" {

            do {

                let data = try readPackage(
                    at: url
                )

                let summary = try PatchPackageCodec.inspect(
                    data
                )

                let decoded: DecodedPatchPackage?

                if let contentKey = try PatchKeyStore.load(
                    for: summary
                ) {

                    decoded = try PatchPackageCodec.decode(
                        data,
                    ...
                    // Only the app's renamed bundled resources
                    // use the internal key; imported packages
                    // remain locked for the user.

                    guard url
                        .deletingPathExtension()
                        .lastPathComponent
                        .hasPrefix("VOLCANO File (")
                    else {
                        decoded = nil
                        continue
                    }
                ...
                if summary.schemaVersion >= 2,
                   let project = decoded?.project {
                    ...
                    do {
                        ...
                        _ = try PatchWorkspaceService.ensureWorkspace(
                            for: project
                        )
                        ...
                        log(
                            "patch: workspace unavailable for \(project.id.uuidString)"
                        )
                    }
                }
                ...
                log(
                    "patch: skipped invalid local package \(url.lastPathComponent)"
                )
            }
        }

        return byID.values.sorted {
            ($0.project?.updatedAt ?? .distantPast)
                >
            ($1.project?.updatedAt ?? .distantPast)
        }
    }
    ...
    static func installImportedPackage(
        ...
        let previousData = try existingURL.map {
            try readPackage(at: $0)
        }
    ...
    private static func sanitizedFilename(
        ...
        let scalars = rawName.unicodeScalars.map {
            allowed.contains($0)
                ? Character(String($0))
                : "-"
        }
