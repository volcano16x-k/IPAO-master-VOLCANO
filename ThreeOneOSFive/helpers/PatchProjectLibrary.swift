...
enum PatchProjectLibrary {
    ...
    static func installBundledPackagesIfNeeded(
        ...
        let nestedURLs = bundle.urls(forResourcesWithExtension: "VOLCANO", subdirectory: "Patches") ?? []
        let flattenedURLs = bundle.urls(forResourcesWithExtension: "VOLCANO", subdirectory: nil) ?? []
        var seen = Set<String>()
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        for url in urls where url.pathExtension.lowercased() == "VOLCANO" {
            do {
                ...
                if let contentKey = try PatchKeyStore.load(for: summary) {
                    ...
                    guard url.deletingPathExtension().lastPathComponent.hasPrefix("VOLCANO File (") else {
                        decoded = nil
                        continue
                    }
                    do {
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension("VOLCANO")
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension("VOLCANO")
                suffix += 1
            }
